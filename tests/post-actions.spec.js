...
test('Create, Edit, Delete post', async ({ page }) => {
    ...
    await page.fill('input[name="email"]', 'pl@gmail.com');
    await page.fill('input[name="password"]', 'patricia123');
    await page.click('button:has-text("Login")');

    await page.fill('input[name="title"]', 'Playwright Post');
    await page.fill('textarea[name="content"]', 'This is a content');
    await page.setInputFiles('input[name="image"]', 'tests/fixtures/test-image.jpg');
    await page.click('button:has-text("Create")');
    await expect(page.locator('text=Post created successfully!')).toBeVisible({timeout: 80000 });
